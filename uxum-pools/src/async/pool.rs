...
use crate::{error::Error, metrics::{Metrics, POOL_METRICS, pool_kv}, r#async::InstrumentablePool};
...
impl<P: InstrumentablePool> InstrumentedPool<P> {
    ...
    /// Get instrumented connection from resource pool.
    pub async fn get(&self) -> Result<InstrumentedResource<P::Resource>, Error> {
        // TODO: instrumentation
        let metrics = self.metrics.clone();
        let now = Instant::now();
        let label = self.label.clone();
        let span = debug_span!("pool_acquire", name = label[0].value.as_str().as_ref());
