...
impl<P: for<'p> InstrumentablePool<'p>> InstrumentedPool<P> {
    ...
    pub fn get(
        ...
        let now = Instant::now();
        let span =
            debug_span!("pool_acquire", name = self.label[0].value.as_str().as_ref()).entered();
    ...
    pub fn get_timeout(
        ...
        .entered();
        let resource = self.pool.get_timeout(timeout)?;
        drop(span);
        self.measure_acquire(now)?;
        Ok(InstrumentedResource::new(
            self.metrics.clone(),
